...
def _edit_address_format(country, inserted_field, reference_field, mode='after'):
    ...
    try:
        position = next(i for i, v in enumerate(address_splitted) if reference_field in v)
    except Exception:
        return False
    if mode == 'after':
        position += 1
...
def post_init_hook(env):
    inserted_field = "industrial_estate_name"
    operations = [
        ('state_name', 'before'),
        ('state_code', 'before'),
        ('city', 'after')
...
def uninstall_hook(env):
    fields = ["industrial_estate_name", "industrial_estate_id"]
    countries = env["res.country"].search([])
    for country in countries:
